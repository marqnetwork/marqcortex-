/**
 * AI-01 Batch 4A — controlled OpenAI production proof.
 *
 * Drives ONE real OpenAI request through the existing governed path and prints
 * what it cost, what it returned and where the money was recorded:
 *
 *   HTTP adapter → control plane → guard → policy → spend guard →
 *   provider selector → OpenAI adapter → usage → spend ledger → audit
 *
 ...
 *
 *   The live call runs only with `--live` AND a configured OpenAI credential
 ...
 *
 *   The spend ceiling is set to a strict proof ceiling (default $0.05), not the
 *   deployment's. The MARQ lifetime ledger is in-memory here, so a proof run
 *   cannot consume the deployment's funded headroom.
 *
 *   Selection is narrowed to the cheapest certified OpenAI model through the
 ...
 *
 *   Failover is off. The proof either reaches OpenAI or fails visibly; it never
 *   quietly reports success over a synthetic mock completion.
 *
 ...
 *   OpenAI adapter reads it from the environment itself; this script only ever
 *   asks the adapter `hasCredentials()`.
 *
 * Usage:
 *   node --experimental-strip-types scripts/openai-live-proof.ts           # preflight only
 *   node --experimental-strip-types scripts/openai-live-proof.ts --live    # preflight + ONE paid call
 */
...
import { createOpenAIProvider, type FetchLike } from '../supabase/functions/server/ai/providers/openaiProvider.ts';
...
/** The cheapest model the certified OpenAI adapter declares. */
const PROOF_MODEL_ID = 'gpt-4o-mini';
const PROOF_ORGANIZATION_ID = 'marq-batch-4a-proof';
 ...
 * the most expensive model the adapter DECLARES, times the attempt allowance.
 * For the chat feature that hold is about $0.106 whatever the administrative
 * model allow list says, because the estimate reads the adapter's catalogue
 * rather than the permitted subset — conservative in the safe direction, and
 * the reason this ceiling is not a penny. Twenty-five cents clears the hold and
 * is still far below the $9 deployment default; the metered cost of the one
 * request it authorises is on the order of $0.00005.
 */
...
function buildProofPlane(realEnv: EnvSource) {
  ...
  const openai = createOpenAIProvider({ env: realEnv, fetchImpl: countingFetch });
  ...
  const plane = createControlPlane({
    ...
    authenticator,
    providers: [
      { adapter: openai, certification: 'certified' },
  ...
  return { plane, admin, openai, mock, config, outbound, sink, env };
}
...
async function main(): Promise<void> {
  ...
  const { plane, admin, openai, mock, config, outbound, sink } = buildProofPlane(realEnv);

  console.log('MARQ CORTEX — AI-01 Batch 4A controlled OpenAI production proof');
  ...
  check(
    'the OpenAI adapter is registered in the one provider registry',
  ...
  const cheapest = [...declared].sort(
    (a, b) =>
      a.promptMicroUsdPer1k + a.completionMicroUsdPer1k -
      (b.promptMicroUsdPer1k + b.completionMicroUsdPer1k),
  )[0];
  check(
    'the cheapest certified OpenAI model is the proof model',
    cheapest?.modelId === PROOF_MODEL_ID,
    `cheapest=${cheapest?.modelId} prompt=${cheapest?.promptMicroUsdPer1k}µUSD/1k ` +
      `completion=${cheapest?.completionMicroUsdPer1k}µUSD/1k`,
  );
  ...
  const credentialsConfigured = openai.hasCredentials();
  check(
    'the OpenAI credential is read only by the adapter',
    true,
  ...
  const openAiHealth = health.providers.find((entry) => entry.providerId === PROVIDER_ID);
  check(
    'the real-request kill switch is reported by the health surface',
    openAiHealth !== undefined,
    `AI_ALLOW_REAL_REQUESTS=${realRequestsPermitted} providerState=${openAiHealth?.state} ` +
...

  {
    ...
    check(
      'with the kill switch OFF, OpenAI is removed from selection and never called',
...

  {
    ...
    await stopped.admin.setEmergencyStop(
      operator,
      true,
      'AI-01 Batch 4A: proving the emergency stop refuses execution',
    );
  ...
  await admin.updateProvider(
    ...
    'AI-01 Batch 4A: restrict the production proof to the cheapest certified model',
  );
  ...
  if (!credentialsConfigured) {
    check(
      'LIVE CALL',
      false,
      'BLOCKED — no OpenAI credential is configured in this environment. ' +
  ...
  const response = await executeAIHttpRequest(plane, chatRequest('cor-batch4a-live'));
  ...
  check(
    'exactly ONE outbound OpenAI call was made',
    outbound.length === 1 && outbound[0].url.startsWith('https://api.openai.com/'),
  ...
  check(
    ...
    `status=${response.status} provider=${String(meta.provider)} model=${String(response.body.model)} ` +
  ...
  await admin.setEmergencyStop(
    operatorAgain,
    true,
    'AI-01 Batch 4A: holding real execution off after the controlled proof',
  );
  const callsBeforeRefusal = outbound.length;
  const refused = await executeAIHttpRequest(plane, chatRequest('cor-batch4a-post-proof'));
  check(
...
function summarize(live: boolean, sink: ReturnType<typeof createMemorySink>): void {
  ...
  const CREDENTIAL_SHAPE = /\b(sk-[A-Za-z0-9_-]{8,}|Bearer\s+\S+|Authorization)\b/i;
