 ...
 *   STORAGE_FORCE_KV_ONLY               kill switch          default "false"
 *   STORAGE_SHADOW_OUTCOME_ENABLED      shadow master toggle default "false"
 *   STORAGE_SHADOW_OUTCOME_ORG_ALLOWLIST  csv org allowlist  default ""
 *   STORAGE_SHADOW_DEFAULT_ORG_ID       default tenant scope default ""
 *   STORAGE_SHADOW_SQL_TIMEOUT_MS       hard SQL timeout     default 250 (cap 2000)
 *   STORAGE_ENVIRONMENT                 telemetry env label  default ""
 */
...
/** Read the raw, unclamped configuration from the environment. */
...
/**
 * Resolve the mode the gateway will ACTUALLY execute for an entity. The runtime
 * has no SQL-primary path, so any resolved mode other than KV_ONLY is clamped
 * down to KV_ONLY (the only active source of truth).
 */
