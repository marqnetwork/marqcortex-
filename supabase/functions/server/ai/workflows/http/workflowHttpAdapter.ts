/**
 * Framework-agnostic HTTP adapter for the workflow engine (AI-01 Batch 3B).
 *
 * The same shape as `ai/http/httpAdapter.ts`, `ai/admin/httpAdapter.ts` and
 * `agents/http/agentHttpAdapter.ts`, and for the same reasons: the whole
 * request/response mapping is a pure function, so status codes, error bodies and
 * — most importantly — the authorization boundary are unit-testable without a
 * server.
 *
 ...
 * without the caller having been resolved to a workflow actor, because dispatch
 * happens inside the function that does the resolving. A route file cannot forget
 * the check — there is nothing in a route file to forget.
 *
 * THE OPERATION IS BOUND BY THE ROUTE TABLE, NEVER READ FROM THE BODY. A caller
 * cannot ask for `run.cancel` at the `run.get` endpoint. That is the same decision
 * the administration adapter made after a review found the alternative pattern —
 * a handler per operation, each repeating its own role comparison — had produced
 * two endpoints with no authentication at all.
 *
 * BODIES ARE READ FIELD BY FIELD. Each operation reads exactly the fields it
 * declares from an unknown body and ignores everything else, so an undeclared key
 * cannot reach the orchestrator. Identifiers are pattern-checked before they are
 * used, because they end up in storage keys and log lines.
 *
 * NO MUTATION OF A DEFINITION. There is deliberately no `workflow.create`,
 * `workflow.update` or `node.add` operation. Customer-created workflows are out of
 * scope, and this is the surface where that scope would arrive.
 */
...
export const WORKFLOW_OPERATION = {
  ...
  runNodes: 'workflow.run.nodes',
  branchState: 'workflow.run.branches',
  pauseRun: 'workflow.run.pause',
  resumeRun: 'workflow.run.resume',
  cancelRun: 'workflow.run.cancel',
  retryRun: 'workflow.run.retry',
  submitApproval: 'workflow.approval.decide',
  listApprovals: 'workflow.approval.list',
  tokenReport: 'workflow.report.tokens',
  costReport: 'workflow.report.cost',
  financialSummary: 'workflow.report.finance',
  savingsSummary: 'workflow.report.savings',
...
export interface WorkflowHttpRequest {
  ...
  /** Path parameters. Pattern-checked before use. */
...
export async function executeWorkflowHttpRequest(
  ...
  try {
    ...
    switch (request.operation) {
      ...
      case WORKFLOW_OPERATION.getWorkflow:
        return ok(service.getWorkflow(actor, requireId(request.workflowId, 'workflowId')));
      ...
      case WORKFLOW_OPERATION.runNodes:
        return ok({
          nodes: await service.getRunNodes(
            actor,
            requireId(request.workflowRunId, 'workflowRunId'),
            request.organizationId,
          ),
      ...
      case WORKFLOW_OPERATION.branchState:
        return ok(
          await service.getBranchState(
            actor,
            requireId(request.workflowRunId, 'workflowRunId'),
            request.organizationId,
          ),
      ...
      case WORKFLOW_OPERATION.retryRun:
        return ok({
          run: await service.retryRun(
            actor,
            requireId(request.workflowRunId, 'workflowRunId'),
            body.reason,
            meta,
          ...
          run: await service.submitApproval(
            actor,
            {
              ...
              workflowApprovalId: requireId(
                request.workflowApprovalId ?? optionalString(body.workflowApprovalId, 128),
                'workflowApprovalId',
              ),
      ...
      case WORKFLOW_OPERATION.tokenReport:
        return ok({
          report: await service.getTokenOptimizationReport(
            actor,
            requireId(request.workflowRunId, 'workflowRunId'),
            request.organizationId,
          ),
      ...
      case WORKFLOW_OPERATION.costReport:
        return ok({
          report: await service.getCostReport(
            actor,
            requireId(request.workflowRunId, 'workflowRunId'),
            request.organizationId,
          ),
      ...
      case WORKFLOW_OPERATION.financialSummary:
        return ok({ summary: await service.getFinancialSummary(actor, request.organizationId) });

      case WORKFLOW_OPERATION.savingsSummary:
        return ok({
          summary: await service.getOptimizationSavingsSummary(actor, request.organizationId),
        });
      ...
      default:
        // Unreachable while the route table binds only declared operations. Present
        // so adding an operation to the union without a case here is a 404 rather
        // than an undefined response.
        throw new AIError('FEATURE_NOT_FOUND', 'Unknown workflow operation.', {
    ...
    return {
      ...
      // `diagnostics` is deliberately absent. The caller gets a precise message, a
      // transport code and — where the failure is a workflow one — the typed failure
      // an operator console can act on. Server-side detail stays in the log and the
      // workflow audit trail, where it is already recorded.
      body: {
        success: false,
