...
**CURRENT STATE (PROVEN).** The single source of truth is `src/system/manifest.ts` — **171 registered nodes** with IDs `MQC-{PAGE|COMP|CORE|SVC|HOOK|TYPE}-{NNN}` and status `LIVE | DEMO | GATED | SYSTEM`. Approximate composition (`ARCHITECT.md` §1/9/11/12): ~10 page/route wrappers; ~89 domain UI components + ~49 shadcn/ui primitives; 36 core engines; frontend/data/intelligence services; 3 contexts + hooks; 7 domain type files. Status distribution (verified): the large majority `LIVE`, 9 `DEMO`, 3 `GATED`, 2 `SYSTEM`.

**Counting rules (so these figures are unambiguous).** A **manifest node** is one key of `manifest.nodes` in `src/system/manifest.ts` — the same population the validator reports as `totalNodes` (`Object.keys(allNodes).length`, `src/system/validate.ts`). A **core engine** (equivalently *deterministic engine*) is a node of type `CORE`, defined by `src/system/types.ts` as *"a deterministic engine module in `/src/app/core`"*. Every figure quoted for either population in this Blueprint uses these rules. Two `.ts` files present in `src/app/core/` are deliberately **not** `CORE` nodes and are therefore excluded from the engine count — `types.ts` (type declarations only) and `blockRegistrySync.ts` (unregistered) — so the directory holds 38 `.ts` files while the registered engine population is 36. Legacy `utils/registryData*.ts` is **orphaned** — do not extend (use the manifest). Viewer: `#/registry` (`RegistryViewer.tsx`); validator: `system/validate.ts`.
...
- Component registry: `src/system/manifest.ts` (171 nodes); config: `src/config/features.ts`, `api.config.ts`, `runtime.ts`.
...
**Current State (PROVEN).** The organizing philosophy is inherited from Part II and is already visible in the codebase's shape: one canonical authority per responsibility (Operating Constitution Art. 14, DNA Ch 22.5), deterministic engines owning math while AI narrates (DNA Ch 17; Art. 6), and capability organized to *strengthen the AI Workforce model rather than fragment it* (DNA Ch 27.10). Today this coherence is realized in engines, services, and the manifest registry (`src/system/manifest.ts`, 171 nodes; §III-26) — not yet in an organization of AI roles.
...
**Current State (PARTIAL).** An AI *capability* exists today, but not an AI *workforce*. **IMPLEMENTED:** feature-scoped assistive intelligence via the Intelligence Gateway — chat, submission analysis, portfolio/narrative, block assist, copilot, objection handling (§III-15) — all narration/assistance, never authority. **IMPLEMENTED:** the deterministic Business Engines that the workforce will sit atop (36 engines, §III-21). **NOT IMPLEMENTED:** an organization of AI workers with identity, categories, lifecycle, and bounded authority; `ai_worker` identity is reserved as *Future* (DNA Ch 8.3; §III-1/§III-18).
...
- **Meaning of "capability" in Part VI.** Part VI uses *capability* at **delivery grain** — a unit of work that moves through the layers (§VI-14), streams (§VI-17), waves (§VI-22), and gates (§VI-18), as in "capability increment" and "capability wave". This is a **planning unit, not a registry entry**. The registered Enterprise Capabilities are enumerated in `MARQ_CORTEX_ENTERPRISE_CAPABILITY_REGISTRY_v1.0.md` (`C0001`–`C0561`; Ontology §10.8); a Part VI capability increment typically spans several of them, and no list in Part VI is a capability registry or a substitute for one.
...
**Current State.**
- **Engineering — IMPLEMENTED (ready).** A working codebase, 36 engines, repositories, migration tooling, and multi-suite tests (`test:smoke/intelligence/database/migration/features`) provide a disciplined, reversible engineering base (Execution Rules; Test Protocol).
...
**Key Findings.**
1. MARQ Cortex is a substantially implemented product (36 engines, 171-node manifest, full journey, live Intelligence Gateway and CORTEX AI, repository/KV layer, migration tooling, test suites, system memory) on a settled, LOCKED constitutional and architectural foundation (§VI-1, §VI-2).
...
**These two are exhaustive.** Hard and Soft are the **only** dependency strengths in MARQ Cortex V1. There is no third strength — no "Optional" dependency class exists, and none may be introduced by any derived artifact (dependency graph, sequencing matrix, completion matrix, progress report, or tooling). Every edge asserted anywhere in the platform's documentation must be classified as exactly one of Hard or Soft. This restates, for execution, the strength axis fixed in Ontology §24.1/§24.4/§24.5, which is the semantic authority for dependency meaning; §24.8 additionally defines a **Conditional** dependency — one that applies only when stated conditions, policies, states, or events hold — which is an *activation* property, not a third strength: a Conditional dependency is still Hard or Soft once its condition is met. Configuration-gated or tenant-gated availability is therefore Conditional, never a separate class.
...
- **Computed by the engines, narrated by AI.** Authoritative results come from the deterministic engine layer (Operating Constitution Art. 6); AI narrates and assists but is never the authority for a computed customer result (§VI-2).
