...
| **Production pre-flight** | — | Executed 2026-09-14 against main `0fae2d6`, read-only. 21 migrations from empty → **30 tables / 96 indexes / 14 composite keys**, matching the documented census. The refusing migration driven through its whole operator path on dirty data: refuses, names every violating relationship in one attempt, aborts clean without reassigning a row, and re-applies after the fix. Both rollbacks round-trip; the index one is idempotent. All seven off-by-default switches verified in code, not prose. Readiness §11 |
| **Checkpoint A — production access and baseline** | — | Executed 2026-09-16 against main `d3fc9f1`, read-only; production never contacted. **BLOCKED**: no Supabase CLI, no credentials, and `*.supabase.co` refused at the egress proxy, so the migration ledger, the refusal precheck and the pre-mutation row counts could not be captured. Found and fixed two runbook defects an operator would have acted on — the admin rotation procedure was a **silent no-op** on an existing account, and the backfill rollback pointed at the schema-rollback directory. Readiness §12. Stale PR #1 closed: no `git merge-base` exists between its head and current main — unrelated histories — and its capability is superseded and extended by the storage module on main |
| **Release bundle budget** | Task §8 | `vite build` warned on every successful run, so nobody read it. Measured and pinned: entry **47 kB gz**, largest lazy chunk **299 kB gz**, whole bundle **1,221 kB gz** across 86 chunks. `npm run test:bundle` is a ratchet over each. `CortexDashboard` deliberately not split for V1 — checked for an accidental heavy import, and there is none |
