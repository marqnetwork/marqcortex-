...
export function createProposalSnapshot(
  ...
  // `BlockLink` names these `entity_type` / `entity_id` (blockEngine §5). Read
  // under any other name the filter matched nothing, so every snapshot froze an
  // empty block set — a Save Version that saved no blocks.
  ...
  const frozenBlocks: FrozenBlock[] = proposalBlocks.map(s => {
    // Approval lives on the revision, not the block: `Block` carries no
    // approver. The accepted revision is the one `current_revision_id` points
    // at, and `null` there means unapproved, which FrozenBlock spells as absent.
    const accepted = s.revisions.find(r => r.revision_id === s.block.current_revision_id);
    return {
      ...
      approved_by:  accepted?.approved_by ?? undefined,
      approved_at:  accepted?.approved_at ?? undefined,
    };
  });
