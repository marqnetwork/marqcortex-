...
// The ExampleCo execution seed that used to live here has moved to
// `@/app/demo/fixtures/executionProject.ts`.
//
// `ExecutionRoute` read it whenever `EXECUTION_STORE` was empty — which is
// every workspace that has not yet converted a proposal — so the Execution
// destination showed a twelve-week "AI Readiness Programme" for a client
// called ExampleCo, complete with owners, due dates, four governance gates and
// one of them already signed off by an "Account Lead". None of it had
// happened. The route shows an honest empty state now, and the seed is a
// demo fixture like every other.
