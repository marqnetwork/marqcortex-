 ...
 *
 * This used to read `const REF = new Date('2026-03-02T00:00:00.000Z')` and
 * count back from there, because it was written alongside sixteen fixture
 * deals dated relative to that day and the fixtures were the only thing it
 * ever filtered. Then Revenue Intelligence was wired to a live backend and the
 * same function kept counting from March.
 *
 * The effect on real data: "Last 30 days" meant "created on or after
 * 2026-01-31", and went on meaning that as the year went by — a window that
 * silently widened by a day every day, never named the dates it was using, and
 * quietly included everything once enough time had passed. The month-on-month
 * comparison was computed from the same reference, so it compared two windows
 * that had both stopped moving.
 *
 * It counts from today. The fixture keeps its own fixed reference, where a
 * deterministic date is the whole point.
 */
...
// The sixteen seeded deals that used to live here have moved to
// `@/app/demo/fixtures/revenueSnapshots.ts`.
//
// They were `export const MOCK_SNAPSHOTS` in this file — a CORE module that
// every revenue surface imports for its aggregation functions — so named
// clients, owners and deal values totalling a pipeline were statically linked
// into the authenticated bundle and one import away from any surface that
// wanted a quick way to have something on screen. The aggregators here are
// real and stay; the deals they were demonstrated on do not belong in them.
...
export function filterSnapshots(
  ...
  const cutoffISO = daysBeforeToday(cutoff);
...
export function prevPeriodSnapshots(
  ...
  const fromISO  = daysBeforeToday(days * 2);
  const toISO    = daysBeforeToday(days);
