...
export const FN_NODES: RegistryNode[] = [
  ...
  },
  {
    ...
    path: '/supabase/functions/server/index.tsx',
    type: 'FN' as any, domain: 'team-auth',
    description: 'Idempotent admin user seeder. Runs on server startup. Reads TEAM_ADMIN_EMAIL/TEAM_ADMIN_PASSWORD/TEAM_ADMIN_NAME from server-side secrets (values are never hardcoded in the frontend). Creates user if not exists.',
    demands: ['MQC-BEF-001'],
    supplies: ['admin user in Supabase Auth (idempotent)'],
    debugNotes: 'Admin credentials come from the TEAM_ADMIN_EMAIL + TEAM_ADMIN_PASSWORD secrets in Supabase (never hardcoded in the frontend). Runs on every cold start but is idempotent.',
    status: 'stable',
  },
  {
